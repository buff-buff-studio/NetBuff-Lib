...
namespace NetBuff.UDP
{
    ...
    public class UDPNetworkTransport : NetworkTransport
    {
        ...
        private class UDPPeer
        {
            ...
            public Stopwatch latencyStopwatch = new();
        ...
        {
            private const int _MAX_PACKET_SIZE = 1020;
            ...
            public Action<int, byte[]> onPacketReceived;
            public Func<int, byte[], ConnectionRequestAnswer> onReceiveConnectionRequest = (_, _) => new ConnectionRequestAnswer{accepted = true};
            ...
            private void _Loop()
            {
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        var received = _socket.ReceiveFrom(buffer, ref remote);
                        ...
                        switch (type)
                        {
                            case 1:
                                if (peer == null)
                                {
                                    ...
                                    Buffer.BlockCopy(buffer, 1, payload, 0, received - 1);
                                    ...
                                    if (answer.accepted)
                                    {
                                        ...
                                        _SendConnectionResponse(remote);
                                    }
                                    else
                            ...
                            case 8:
                                ...
                                _InternalSend(new ReadOnlySpan<byte>(discovererAnswer), remote);
                                break;

                            case 3:
                                if (peer != null)
                                {
                                    var reason = Encoding.UTF8.GetString(buffer, 1, received - 1);
                            ...
                            case 10:
                                if (peer != null)
                                {
                                    ...
                                    Buffer.BlockCopy(buffer, 1, data, 0, received - 1);
                            ...
                            case 13:
                                if (peer != null)
                                {
                                    var seq1 = (buffer[1] << 24) | (buffer[2] << 16) | (buffer[3] << 8) | buffer[4];

                                    //send ack
                                    buffer[0] = 14;
                                    _InternalSend(new ReadOnlySpan<byte>(buffer, 0, 5), remote);
                                    ...
                                    if (!peer.receivedReliable.ContainsKey(seq1))
                                    {
                                        ...
                                        Buffer.BlockCopy(buffer, 5, b, 0, received - 5);
                            ...
                            case 12:
                                if (peer != null)
                                {
                                    var seq2 = (buffer[1] << 24) | (buffer[2] << 16) | (buffer[3] << 8) | buffer[4];
                                    ...
                                    if (!peer.receivedReliable.ContainsKey(seq2))
                                    {
                                        ...
                                        Buffer.BlockCopy(buffer, 6, b, 0, received - 6);
                            ...
                            case 14:
                                if (peer != null)
                                {
                                    var sequence = (buffer[1] << 24) | (buffer[2] << 16) | (buffer[3] << 8) | buffer[4];
            ...
            private void _InternalSend(ReadOnlySpan<byte> span, EndPoint remote)
            {
                _socket.SendTo(span.ToArray(), remote);
            }
            ...
            private void _SendDisconnectRequest(string reason, EndPoint remote)
            {
                ...
                Buffer.BlockCopy(reasonBytes, 0, _buffer, 1, reasonBytes.Length);

                _InternalSend(new ReadOnlySpan<byte>(_buffer, 0, 1 + reasonBytes.Length), remote);
                _RemoveClient(remote, reason);
            }
            ...
            {
                _buffer[0] = 2;
                _InternalSend(new ReadOnlySpan<byte>(_buffer, 0, 1), remote);
            }
            ...
            public void SendPacket(ReadOnlySpan<byte> span, bool reliable, int clientId)
            {
                var peer = _peers.GetValueOrDefault(clientId);
                ...
                SendPacket(span, reliable, peer.endPoint);
            }

            public void SendPacket(ReadOnlySpan<byte> span, bool reliable, EndPoint remote)
            {
                var peer = _peersByIp.GetValueOrDefault(remote);
                ...
                if (reliable)
                {
                    if (span.Length > _MAX_PACKET_SIZE)
                    {
                        ...
                        for (int i = 0; i < fragments; i++)
                        {
                            ...
                            var fragment = span.Slice(i * _MAX_PACKET_SIZE, fragmentSize);
            ...
            private void _SendReliableHeader(ReadOnlySpan<byte> fragment, UDPPeer peer, byte count)
            {
                var id = peer.nextSequenceNumber++;
                peer.sentPacketCount++;

                byte[] buffer = new byte[fragment.Length + 6];
                buffer[0] = 12;
                ...
                Buffer.BlockCopy(fragment.ToArray(), 0, buffer, 6, fragment.Length);

                peer.sentReliable.Add(id, new ReliableSent { sentTicks = DateTime.Now.Ticks, data = buffer });

                _InternalSend(buffer, peer.endPoint);
            }

            private void _SendReliable(ReadOnlySpan<byte> fragment, UDPPeer peer)
            {
                var id = peer.nextSequenceNumber++;
                peer.sentPacketCount++;

                byte[] buffer = new byte[fragment.Length + 5];
                buffer[0] = 11;
                ...
                Buffer.BlockCopy(fragment.ToArray(), 0, buffer, 5, fragment.Length);
            ...
            private void _SendUnreliable(ReadOnlySpan<byte> fragment, UDPPeer peer)
            {
                peer.sentPacketCount++;

                _buffer[0] = 10;
                Buffer.BlockCopy(fragment.ToArray(), 0, _buffer, 1, fragment.Length);
                _InternalSend(new ReadOnlySpan<byte>(_buffer, 0, 1 + fragment.Length), peer.endPoint);
            }
            ...
            public void PoolEvents()
            {
                ...
                for (var i = 0; i < _peers.Count; i++)
                {
                    ...
                    if (now - peer.lastKeepAlive > _KEEP_ALIVE_TIME)
                    {
                        ...
                        _buffer[0] = 5;
                        _buffer[1] = (byte)(peer.latency >> 8);
                        _buffer[2] = (byte)peer.latency;
                        
                        _InternalSend(new ReadOnlySpan<byte>(_buffer, 0, 3), peer.endPoint);
                    ...
                    for (var j = 0; j < peer.sentReliable.Count; j++)
                    {
                        ...
                        if (now - packet.sentTicks <= _RESEND_TIME) 
                            continue;
                        _InternalSend(packet.data, peer.endPoint);
                    ...
                    while (peer.receivedUnreliable.Count > 0)
                    {
                        ...
                        onPacketReceived?.Invoke(peer.id, data);
                    }
                    ...
                    while (peer.receivedReliable.ContainsKey(peer.expectedSequenceNumber))
                    {
                        ...
                        {
                            peer.receivedPacketCount++;
                            onPacketReceived?.Invoke(peer.id, packet.data);
                        ...
                        {
                            var length = 0;
                            for (int j = peer.waitingFragmentSince; j <= peer.waitingFragmentUntil; j++)
                                length += peer.receivedReliable[j].data.Length;
        ...
        private class UDPClient
        {
                ...
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        switch (type)
                        {
                            ...
                            case 5:
                                latency = (buffer[1] << 8) | buffer[2];
                                //send back
                                _InternalSend(new ReadOnlySpan<byte>(buffer, 0, 3));
                            ...
                            case 11:
                            case 13:
                                int seq1 = (buffer[1] << 24) | (buffer[2] << 16) | (buffer[3] << 8) | buffer[4];

                                //send ack
                                buffer[0] = 14;
                                _InternalSend(new ReadOnlySpan<byte>(buffer, 0, 5));
                                _lastReceivedTicks = DateTime.Now.Ticks;
                            ...
                            case 12:
                                int seq2 = (buffer[1] << 24) | (buffer[2] << 16) | (buffer[3] << 8) | buffer[4];
                            ...
                            case 14:
                                int seq = (buffer[1] << 24) | (buffer[2] << 16) | (buffer[3] << 8) | buffer[4];
            ...
            private void _InternalSend(ReadOnlySpan<byte> span)
            {
                _socket.SendTo(span.ToArray(), _server!);
            }

            private void _SendConnectionRequest(byte[] payload)
            {
                _buffer[0] = 1;
                Buffer.BlockCopy(payload, 0, _buffer, 1, payload.Length);
                _InternalSend(new ReadOnlySpan<byte>(_buffer, 0, 1 + payload.Length));
            }
            ...
            private void _SendDisconnectRequest(string reason)
            {
                ...
                var reasonBytes = Encoding.UTF8.GetBytes(reason);
                Buffer.BlockCopy(reasonBytes, 0, _buffer, 1, reasonBytes.Length);
                _InternalSend(new ReadOnlySpan<byte>(_buffer, 0, 1 + reasonBytes.Length));
                
                _actions.Enqueue(() => onDisconnected?.Invoke(reason));
            }
            

            public void SendPacket(ReadOnlySpan<byte> span, bool reliable)
            {
                if (reliable)
                {
                    if (span.Length > _MAX_PACKET_SIZE)
            ...
            private void _SendReliableHeader(ReadOnlySpan<byte> fragment, byte count)
            {
                ...
                _InternalSend(buffer);

            }

            private void _SendReliable(ReadOnlySpan<byte> span)
            {
                var id = _nextSequenceNumber++;
                sentPacketCount++;

                byte[] buffer = new byte[span.Length + 5];
                ...
                Buffer.BlockCopy(span.ToArray(), 0, buffer, 5, span.Length);

                _sentReliable.Add(id, new ReliableSent { sentTicks = DateTime.Now.Ticks, data = buffer });
                _InternalSend(buffer);
            }

            private void _SendUnreliable(ReadOnlySpan<byte> span)
            {
                sentPacketCount++;

                _buffer[0] = 10;
                Buffer.BlockCopy(span.ToArray(), 0, _buffer, 1, span.Length);
                _InternalSend(new ReadOnlySpan<byte>(_buffer, 0, 1 + span.Length));
            }
            ...
            public void PoolEvents()
            {
                ...
                while (_receivedReliable.ContainsKey(_expectedSequenceNumber))
                {
                    ...
                    {
                        receivedPacketCount++;
                        onPacketReceived?.Invoke(packet.data);
                    ...
                    else if (_expectedSequenceNumber == _waitingFragmentUntil)
                    {
                        ...
                        for (var i = _waitingFragmentSince; i <= _waitingFragmentUntil; i++)
                            length += _receivedReliable[i].data.Length;
                ...
                {
                    if (_receivedReliable.ElementAt(i).Key > _expectedSequenceNumber)
                        break;

                    //print some packet has been lost
                    Debug.LogError("Packet lost");
                }
            }
        }
        #endregion
        ...
        private void Update()
        {
            if (_client != null)
            {
                ...
                foreach (var data in _ProcessQueue(_clientQueueUnreliable, 1020))
            ...
            if (_server != null)
            {
                ...
                foreach (var client in _clients.Values)
                {
                    ...
                    foreach (var data in _ProcessQueue(client.queueUnreliable, 1020))
